...
\jtitle[和文題目（内容梗概／目次用）]%	% 和文題目（内容梗概／目次用）
	{和文題目}	% 和文題目
\etitle{英文題目}	% 英文題目
\jauthor{伊豆 真哉}				% 和文著者名
\eauthor{Shinya Izu}			% 英文著者名
\supervisor{川原 純 教授}			% 指導教員名
...
\subsection{DAGの先行頂点集合と後続頂点集合} %\label{def_PD}
...
\subsection{有向グラフのembedding} %\label{def_PD}

\begin{definition*}[embedding][5★]
...
\subsection{パス分解とパス幅} %\label{def_PD}

\begin{definition*}[パス分解]★
...
\subsection{有向パス分解と有向パス幅} %\label{def_PD}

\begin{definition*}[有向パス分解]★
...
\subsection{DAGパス分解とDAGパス幅} %\label{def_DAG-PD}

\begin{definition*}[DAGパス分解]★
...
なお，[★]では\ref{original}を以下のように定義していることに注意する．本研究ではアルゴリズムの構築のしやすさの観点から上記の定義を用いるものとする．
\begin{enumerate}
    \item ある $i$ $(i \geq 2)$ があり，$u, v \in X_i$, $u \notin X_{i-1}$ \label{original}
\end{enumerate}

以下では各頂点集合$X_i$をバッグと呼ぶ．また，DAGパス幅を以下のように定める．
...
DAGパス幅はグラフがどれだけ有向パスに近い構造をしているかを表すパラメータであり，DAGパス幅の値が小さいほど有向パスに近い構造をしていることを表す．一般の有向グラフに対してDAGパス幅を求める問題はNP困難である．
...
\subsection{nice DAGパス分解} %\label{def_DAG-PD}

\begin{definition*}[nice DAGパス分解]★
...
\begin{definition*}[Black Pebbling game]★
...
一般のDAGに対してペブリング数を求める問題はPSPACE完全である[The pebbling problem is complete in polynomial space★]．以下ではDAG $G$に対するペブリング数を$Peb(G)$と表現する．グラフ小石ゲームは，Proof of Space [6★]とよばれるブロックチェーン技術の中で用いられる．Proof of Spaceは空きディスク容量をいくら保持しているかを証明する手法であり，入力DAGが空きディスク容量に対応する．またペブリング数は同時に使用するメモリの量に対応する．ペブリング数が大きいほど，より多くのメモリやデータを使うため，その証明が難しいことを表す．すなわち証明の安全性が高くなることを表す．
...
one-shot BPについても同様にペブリング数を定めることができる．一般のDAGに対してone-shot BPのペブリング数を求める問題はNP困難である[★]．
...
本節では，DAG上の4つのNP困難問題であるDirected Dominating Set Problem，Max Leaf Outbranching Problem，Disjoint Path Problem，Directed Steiner Tree ProblemのDAGパス幅を用いたパラメータ化アルゴリズムを紹介する．これらの問題は有向木幅や有向パス幅に対してはW[1]-hardであるが，DAGパス幅を用いることで容易にパラメータ化アルゴリズムを構築することができる．
...
DiDS problemとは，$G$のmDiDSのサイズを求める問題である．Robertら[2★]は以下を示した．

\begin{theorem}[計算量クラス]
    DiDS problemは$G$がDAGの場合でもNP-hardである．
\end{theorem}
...
\begin{theorem}[計算量クラス]
    Disjoint Path problemはNP-hardである．
\end{theorem}
...
ここで$v \in V$に対し，$pred(v), suc(v)$を以下のように定める．
...
DST problemとは，$G$のminimum-DSTの総重みを求める問題である．各枝の重みが1である場合のDST problemをunit-cost DST problemと呼ぶ．Ganianら[11★]はunit-cost DST problemがDAGの場合でもNP-hardであることを示した．これより，ただちにDAG上のDST problemがNP-hardであることが示される．

\subsubsection{FPTの構成}
Fominら[12★]は枝重み付き無向シュタイナー木問題に対し，ターミナル$R$のサイズ$k$をパラメータとした$2^{O(k\log k)}$のFPTアルゴリズムを与えた．これはDST problemに拡張可能である．一方で，入力グラフの基礎グラフに対する木幅をパラメータとしたFPTは，我々の知る限り示されていない．本研究ではDAGパス幅$w$をパラメータとするFPTを構築し，計算量が$O(2^w(k+w)n + n^2)$であることを示す．
...
ここで$v \in V$に対し，$pred(v)$を以下のように定める．
...
\section{DAGパス幅の$O(\log ^2 n)$近似アルゴリズム} %\label{sec_FPTforPathwidth}
本節では頂点数$n$，最大出自数$k$，パス幅$pw$であるDAG $G$に対し，幅が高々$O(k \cdot pw \log ^2 n)$であるDAGパス分解を与えるアルゴリズムを紹介する．

\subsection{定義と補題}
以下でseparatorに関する定義と補題を与える．

\begin{definition*}[DAG edge-separator][1★]
...
\begin{proof} 
$G=(V, E)$の最適なnice DAG-PDを$P=(X_1, X_2, \dots , X_s)$とする．$P$のDAG-pathwidthは$pw$である．$G$の各頂点を$P$でintroduceされた順に$v_1, v_2, \dots , v_n$とする．$n$が偶数のとき，$v=v_{\frac{n}{2}}$，$n$が奇数のとき，$v=v_{\frac{n+1}{2}}$とし，$v$が初めてintroduceされたバッグを$X_i$とする．$A= X_1 \cup X_2 \cup \dots \cup X_i, B= X_{i+1} \cup X_{i+2} \cup \dots \cup X_s$とする．例として★★Fiture\ref{fig:3}を示す．このとき$Xi$から出て$B$に入る枝の集合を$S$とすると，DAG-PDのルール3より$G'=(V, E \backslash S)$には$A$と$B$の間に枝はなく，$v$の選び方により$|A| \geq \frac{1}{3}|V|, |B| \geq \frac{1}{3}|V|$を満たす．またDAG-PDのルール2より$A$と$B$の間の枝は$A$から出て$B$に入る向きである．よって$S$は$G$の$\frac{1}{3}$-balanced DAG edge-separatorである．$G$の最大出次数が$k$であることに注意すると，$S$のコスト$C(S)$について以下の不等式が成り立つ．
\begin{align*}
...
[Inapproximability of Treewidth, One-Shot Pebbling, and Related Layout Problems★]では，以下のようにone-shot BPの近似アルゴリズムの存在を示している．
...
\section{$O(ld^t)$の幅を求めるFPTアルゴリズム} %\label{sec_FPTforPathwidth}

本節では最大出次数$d$，根の個数$l$であるDAG $H$と非負整数$t$が与えられたとき，$O(ld^t)$の幅を持つDAG-PDを出力するか，$H$のパス幅が$t$よりも大きいことを示す証拠を与えるFPTアルゴリズムを紹介する．

\subsection{無向グラフにおけるパス幅の近似アルゴリズム}
Kevinらは[5★]で以下の補題を示している．
...
\subsection{近似率$O(ld^t)$の多項式時間アルゴリズム}
...
\begin{algorithm}[H]
    ...
    \WHILE{$|X_i| < |M_{t, d, l}|$ and $H'$ has at least one blue vertex}
...
\begin{proof}
    Theorem\ref{approximation3}より，FindEmbeddingは$H$のDAGパス幅が$t$より大きい証拠として完全$d$分木の$H$へのembeddingを出力するか，幅が高々$O(ld^t)$のパス分解を出力する．以下ではFindEmbeddingが多項式時間で終了することを示す．GrowTokenTreeでは列1のwhileの条件の判定は高々$O(dn)$かかり，これを高々$|M_{t, d, l}| = O(d^t)$回繰り返す．またFindEmbeddingの列6の$T$の削除によって，$T$が置かれていた頂点はredのままであるため，この操作は高々$O(n)$回行われることに注意すると，列4のwhileもまた高々$O(n)$回行われる．さらに列10のwhileは明らかに高々$d^2$回繰り返す．他のステップは$O(1)$の計算時間で処理される．以上よりアルゴリズムは$d, l$がある定数以下であるとすると$O(n^2)$で停止する．
\end{proof}
...
本節では，有向グラフ$G$のDAG-treewidthの計算がNP-hardであることを示す．

\begin{theorem}\label{np-hard}
    有向グラフ$G$が与えられたとき，$G$のDAG-treewidthを計算する問題はNP-hardである．
\end{theorem}

Theorem\ref{np-hard}を証明するために，まず以下のLemmaを示す．
...
上のLemmaを用い，[3]の手法を参考にしてTheorem\ref{np-hard}を示す．

\begin{proof}[proof of Theorem\ref{np-hard}]
...
\subsection{reverse DAG Tree Decomposition}
DAG-TD $T$をFPTアルゴリズムに利用する場合，$T$の葉から動的計画法を開始するとFPTを構築しやすい．本節では，よりFPTアルゴリズムの構築に適したDAG-TDであるreverse DAG Tree Decomposition(rDAG-TD)を定義する．


\begin{definition*}
 有向グラフ$G=(V, E)$のreverse DAG Tree Decomposition(rDAG-TD)とは，反有向木$T$と，$T$の各ノード$t$について$\beta(t) \subseteq V$であるような$\beta$とのペア$(T, \beta)$である．$(T, \beta)$は以下の3つを満たす．なお，$T$の根の数を$n_l$とする．
...
rDAG-TDの例をFigure★★\ref{fig:6}で示す．以下でreverse DAG-treewidthを定義する．

\begin{definition*}[reverse DAG-treewidth]
    あるrDAG-TD $(T, \beta)$のwidthを$\max_{t \in T}|\beta(t)|$と定義する．このとき，有向グラフ$G$のreverse DAG-treewidth(rDAG-treewidth)とは，$G$に対する全てのrDAG-TDのうち，widthが最小であるrDAG-TDのwidthである．
\end{definition*}

以降では，$G$のreverse DAG-treewidthを$rtw(G)$と表す．


\subsection{nice reverse DAG-TD}

本節では，nice DAG-TDと同様にnice reverse DAG-TD(nice rDAG-TD)を説明する．

\begin{definition*}
 $G=(V, E)$を有向グラフとし，$(T, \beta)$を$G$のrDAG-TDとする．$(T, \beta)$が以下の２つを満たすとき，$(T, \beta)$は$G$のnice reverse DAG-TD(nice rDAG-TD)であるという．
...
nice rDAG-TDの例をFigure★★\ref{fig:7}で示す．あるrDAG-TDが与えられたとき，widthが同じである$G$のnice rDAG-TDを効率よく構築することができる．

\begin{theorem}
    $(T, \beta)$が有向グラフ$G=(V, E)$のrDAG-TDであるとする．また$(T, \beta)$のwidthを$w$とする．このとき，widthが$w$であるような$G$のnice rDAG-TDを多項式時間で構築できる．
\end{theorem}

\begin{proof}
    $(T, \beta)$に対し，以下の手順を行いnice rDAG-PDを得る．
    ...
    以上の操作によってできるrDAG-TDは，widthが高々$w$であり，また$G$のnice rDAG-TDである．この構築は$G$の頂点数の多項式時間で行える．
\end{proof}
...
本節では，有向グラフ$G$のrDAG-treewidthの計算がNP-hardであることを示す．

\begin{theorem}\label{r-np-hard}
    有向グラフ$G$が与えられたとき，$G$のrDAG-treewidthを計算する問題はNP-hardである．
\end{theorem}

Theorem\ref{r-np-hard}を証明するため，次で定義されるreverse DAG-pathwidthの計算がNP-hardであることを示す．


\begin{definition*}
     $G=(V, E)$とする．$G$のreverse DAG Path Decomposition(rDAG-PD)とは，以下の3つの条件をみたすようなVの各部分集合$ X_i (i = 1, 2,  \ldots, s)$の列 $X=(X_1, X_2,  \ldots, X_s)$ である．
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item $u, v \in X_1$
        \item ある $i$ $(i \geq 2)$ があり，$u \in X_i$, $u \notin X_{i-1}$, $u \in X_i$が成り立つ．
        \end{itemize}
        \item 任意の$ i, j, k (1 \leq i \leq j \leq k \leq s)$ について， $X_i \cap X_k \subseteq X_j$ が成り立つ．すなわち，各頂点$v$について，$v$は$X$上で非空な部分道を誘導する．
    \end{enumerate}
    あるrDAG-PD $P = (X_1, X_2,   \ldots, X_s)$のwidthを$\max_{1 \leq i \leq s} {X_i}$と定義する．このとき，有向グラフ$G$のreverse DAG-pathwidthとは，$G$に対する全てのrDAG-PDのうち，widthが最小であるrDAG-PDのwidthである．
    
\end{definition*}
 
以降では，$G$のreverse DAG-pathwidthを$rpw(G)$と表す．





\begin{lemma}\label{rDAG-PD}
    有向グラフ$G$，整数$k$が与えられたとき，$rpw(G) \leq k$が成り立つかを判定する問題はNP-completeである．
\end{lemma}

\begin{proof}
    $G$のあるrDAG-PDが与えられたとき，そのwidthが$k$以下かどうかを判定することは明らかに多項式時間で行えるため，この問題はNPに属する．また，$G$の全ての枝の向きを逆にした有向グラフを$G'$とする．$G$に対してwidthが$pw(G)$であるDAG-PD $P$が与えられたとき，$G'$に対して最小のwidthをもつrDAG-PD $P'$は，DAG-PDとrDAG-PD定義より$P$と等しい．したがって$P'$のwidthは$pw(G)$であるため，$rpw(G')= pw(G)$が成り立つ．以上よりLemma\ref{rDAG-PD}が示される．
\end{proof}
...
\begin{proof}
    rDAG-PD, rDAG-TDの定義より，widthが$rpw(G)$である$G$のrDAG-PDは$G$のrDAG-TDでもある．よって$rtw(G) \leq rpw(G)$が成り立つ．一方，widthが$rtw(G)$である$G$のnice rDAG-TDを$(T, \gamma)$とし，$G$のただ一つの強連結成分であるsinkを$A \subseteq V$とする．このときnice rDAG-TDの定義より，ある根$r$とその子$r_c$があり，$\gamma(r) = \varnothing, \gamma(r_c) = A$が成り立つ．このとき$T$はただ一つの根$r$をもつことがいえる．なぜならば，もし$T$が$r$以外の根$r'$ $(\gamma(r') = \varnothing)$をもつと仮定すると，$r'$の子はバッグに$A$を含まないため，$G$に$A$以外のsinkが存在することになり，矛盾するからである．したがって$T$はunionのノードを持たず，任意の$t \in T$は入次数と出次数が高々1である．したがってrDAG-PDとrDAG-TDの定義より，$(T, \gamma)$は$G$のDAG-PDでもある．これより$rpw(G) \leq rtw(G)$がいえる．以上より，$tw(G) = pw(G)$が示される．
\end{proof}
...
最後に定義\ref{r-np-hard}を示す．

\begin{proof}
    ある有向グラフ$G = (V, E)$が与えられたとき，定義\ref{np-hard}の証明と同様の手順で$|V'| = 2|V| + 1, |E'| = \frac{1}{2}(|V|^2 + 3|V|) + 2|E|$である有向グラフ$G' = (V', E')$ を構成する．また$G'$の頂点集合$A, B$を同様に$A = \{v_i | i =1, 2, \dots, n\}, B = \{v'_i | i =1, 2, \dots, n\}$と定める．このとき$V' = A \cup B \cup \{s\}$とできる．$G'$は$G$のサイズの多項式時間で構成できる．
    
    次に，widthが$rpw(G)$である$G$のrDAG-PDを$(P, \beta)$ $(P = p_1, p_2, \dots, p_r)$とする．各$m$に対し，$G'$の頂点集合$\gamma(p_m)$を以下のように構成する．
    ...
    以上の操作によってできる$(P', \gamma)$ $(P' = (p_0, p_1, p_2, \dots, p_r))$が$G'$のrDAG-PDになっていることを示す．Figure★★\ref{fig:4}の$G, G'$に対するrDAG-PD $(P, \beta), (P', \gamma)$の構成例をFigure★★\ref{fig:5}で示す．$\gamma$の構成法に注意すると，$A \subseteq \gamma(p_r), B \subseteq \gamma(p_1)$がいえ，さらに$r \in \gamma(p_0)$であることから，$\sum_{m=0}^{r} \gamma(p_m) = V'$が成り立つ．よって$(P', \gamma)$はrDAG-PDのルール1を満たす．また$\gamma$の構成法に注意すると，任意の$v \in V' \backslash \{r\}$に対して，$v$を含む全てのバッグが$(p_1, p_2, \dots, p_r)$上で非連結になることはなく，また$B \cup \{r\} \subseteq \gamma(p_0)$より，任意の$v \in V'$に対して，$v$を含む全てのバッグが$(p_0, p_1, p_2, \dots, p_r)$上で非連結になることはない．よって$(P', \gamma)$はrDAG-PDのルール3を満たす．さらに定義\ref{np-hard}の証明と同様の議論により，$(P', \gamma)$がrDAG-PDのルール2を満たすことがいえる．以上より$(P', \gamma)$は$G'$のrDAG-PDである．このとき$(P', \gamma)$の幅は$rpw(G)+n$である．

    さらに\ref{np-hard}の証明と全く同様の議論により，$(P', \gamma)$が$G'$に対してwidthが最小のrDAG-PDであることがいえる．ここで$G'$はsinkを一つしか持たないことに注意すると，Lemma\ref{r-sink}より$rtw(G') = rpw(G')$がいえ，$rtw(G) = rpw(G) + n$が成り立つ．以上より定義\ref{r-np-hard}が示される．
\end{proof}
...
\section{DAG-treewidthを用いたFPTアルゴリズム} %\label{sec_HowToUseTreewidth}
DAG-treewidthを使ったNP-hardな問題に対するFPTアルゴリズム
