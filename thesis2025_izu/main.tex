...
\begin{proof}
    ...
    \begin{enumerate}
        \item ある頂点$v' \in \mathsf{suc}(u')$が存在し，$v'$のある親$w'$がblueである．\label{cond11}
        \item $T'$がuntokenedな子をもたない．\label{cond22}
    \end{enumerate}

    なお$u'$の子が存在しない場合は考えなくてよい．なぜならばこのとき$u'$に置かれたトークンをremoveでき，(3)の状態にならないからである．条件\ref{cond22}が成り立つならば，$T'$が葉であるか$T'$の子がすべてtokenedであるかのいずれかであるが，これらはいずれも成り立たない．なぜならば仮定より$P'$の終点$T'$は葉でなく，また$T'$がtokenedな子をもてば$P'$が最長のパスであることに矛盾するからである．したがって条件\ref{cond11}が成り立つ必要がある．ここで根トークン$\lambda$が置かれた頂点を$r_0$とする．$H'$上で$r_0$の子孫によって誘導される部分グラフを$G_{r_0}$とし，$G_{r_0}$上の頂点のうち，$M_{t, d, l}$上で高さが$h\ (1 \leq h \leq h(M_{t, d, l}))$であるトークンが置かれた頂点の集合を$V_h$とする．また$\mathcal{V} = \bigcup_{1 \leq h \leq h(M_{t, d, l})} V_h$とする．仮定より$u' \in V_{h'}\ (1 \leq h' \leq h(M_{t, d, l})-1)$である．またLemma~\ref{blue_reachable}より，blueな$v', w'$は共に$r_0$から到達可能であるため$G_{r_0}$に含まれている．また$\mathsf{GrowTokenTree}$のトークンの置き方からblueな頂点の子はredにならないことに注意すると，$v', w'$及びこれらの子孫はすべて$\mathcal{V}$に含まれていない．ここで$w'$について次のいずれか一方が必ず成り立つ．

    \begin{enumerate}
        \item $w'$の親がすべて$\mathcal{V}$に含まれる．\label{cond_a}
        \item $w'$の親で$\mathcal{V}$に含まれないblueな頂点が少なくとも１つ存在する．\label{cond_b}
    \end{enumerate}

    なお，$w'$の親で$\mathcal{V}$に含まれないredな頂点は存在しない．なぜならばLemma~\ref{blue_reachable}と同様の議論により，$w'$の親はそれぞれ(a)blueか，(b)redでトークンが置かれているかのいずれかしかないからである．したがって1つでも(a)である親が存在すれば条件\ref{cond_b}となり，それ以外であれば条件\ref{cond_a}となる．もし条件\ref{cond_a}が成り立つならば，$w'$にはトークンを置くことができるはずである．なぜならば，パス$P'$が最長であることから$w'$の親はすべて$V_1, V_2, \dots, V_{h'}$のいずれかに必ず含まれ，かつ$w'$の各親に置かれたトークンは$H'$の最大出次数である$d$個の子をそれぞれ持っている．よって$w'$の各親に置かれたトークンは少なくとも1つのuntokenedな子を持っているはずであり，このようなトークンを$w'$に置くことができるからである．したがって条件\ref{cond_a}が成り立つならば$w'$がblueであることと矛盾する．これより条件\ref{cond_b}が成り立つ必要がある．このとき条件\ref{cond_b}を満たすような$w'$のblueな親を$w''$とすると，$w''$についても同様の議論によって条件\ref{cond_b}を満たすblueな親$w'''$が存在する必要がある．この議論を繰り返すと，$H'$の頂点数は有限個であるため，いずれ必ず条件\ref{cond_a}を満たすblueな頂点$w^{'\dots '}$が存在する．したがって必ず矛盾が生じるため，背理法により$M_{t, d, l}$でtokenedなトークン集合上に根から葉までのパス$P = (\lambda \cdot m_1 \cdot m_2 \cdot \dots m_{h(M_{t, d, l})-1})$が少なくとも1つは存在する．
