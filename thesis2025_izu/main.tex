...
本節ではDAG-PDを一般化した概念のDAG Tree Decompositionを導入する．
...
以降では，$G$のDAG-pathwidth, DAG-treewidthをそれぞれ$\mathsf{pw}(G), \mathsf{tw}(G)$と表す．DAG-treewidthを求める問題の計算量クラスについて，以下を示す．
...
最後にnice DAG-PDと同様にnice DAG-TDについても定義する．

\begin{definition*}[nice DAG-TD]
...
\begin{theorem}[nice DAG-TDの効率的な構築]
...
\begin{definition*}[co-DAG-treewidth]
    あるco-DAG-TD $(T, \beta)$のwidthを$\max_{t \in T}|\beta(t)|$と定義する．このとき，有向グラフ$G$のco-DAG-treewidthとは，$G$に対する全てのco-DAG-TDのうち，widthが最小であるco-DAG-TDのwidthである．
\end{definition*}

以降では，$G$のco-DAG-treewidthを$\mathsf{rtw}(G)$と表す．co-DAG-treewidthを求める問題の計算量クラスについて，以下を示す．
...
最後にnice co-DAG-TDについても定義する．

\begin{definition*}[nice co-DAG-TD]
...
\begin{theorem}[nice co-DAG-TDの効率的な構築]
